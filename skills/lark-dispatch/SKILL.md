...
| **自动触发（cron）** | 定时检查近期会议，自动执行分发 | 每日分发、团队级运营 |

### 配置自动触发（推荐：每日早晨分发昨日会议）

使用 Cola cron 或系统 crontab 实现定时调度：

**方式一：Cola cron（推荐，开箱即用）**

在 Cola 中说：
> 帮我设置一个每天早上 9 点的定时任务，把昨天的会议自动分发一下

Cola 会自动创建 cron，每天 09:00 触发 lark-dispatch，处理前一天的所有会议。
...
在自动触发场景下，Step 4 的确认通过**飞书消息卡片**呈现：
- Agent 将提取结果以卡片形式发送到用户的飞书
- 用户点击"确认分发"后，才执行 Step 5
- 用户点击"修改"，进入对话修改流程
- 24 小时内未响应，自动跳过（在分发报告中标注"待确认超时"）
...
当 lark-retro 生成周期报告后，可自动触发 lark-dispatch 对报告内容进行二次分发，实现知识从"整合"到"到达"的完整闭环。
